...
class State333:
    ...
    def get_available_actions(self):
        ...
        available_actions = [i for i in range(9) if self.data.flatten()[i] == 0]
...
class TicTacToe333:
    def __init__(self, board_rows=3, board_cols=3):
        self.board_rows = board_rows
        self.board_cols = board_cols
        self.BOARD_SIZE = self.board_cols * self.board_cols
        self.current_state = None  # 현재 상태 관리
        self.current_agent_int = None  # 현재 에이전트(플레이어) 관리
        self.ALL_ACTIONS = [0, 1, 2, 3, 4, 5, 6, 7, 8]

        self.INITIAL_STATE = State333()  # 초기 상태 설정
